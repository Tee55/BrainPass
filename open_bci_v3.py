...
class OpenBCIBoard(threading.Thread):
	"""
	...
	def start_streaming(self, callback, lapse=5):
		"""
		...
		while self.streaming:
			...
			if self.daisy:
				...
				# even sample: concatenate and send if last sample was the fist part, otherwise drop the packet
					...
					whole_sample = OpenBCISample(
						sample.id, sample.channel_data + self.last_odd_sample.channel_data, avg_aux_data)
